...
| [`observer-driven-scale-chart-compiler/`](observer-driven-scale-chart-compiler/) | S0/S1 complete; S4 disposition narrow | exact backward task-repair semantics, oracle-free Airy and strict held-out monomial-chart certificates; no independent-domain or economy theorem | [S0/S1 results](observer-driven-scale-chart-compiler/02-s0-s1-results.md); next freeze a generic analytic-germ adapter, then run raw Bessel discovery, negative/ambiguous controls, and the same-information baseline gate |
