...
| [`observer-driven-scale-chart-compiler/`](observer-driven-scale-chart-compiler/) | S2 local-chart gate passed; disposition narrow | exact backward task-repair semantics plus frozen Airy, held-out cusp, and registered raw-Bessel-to-local-chart certificates; no uniform integral, comparative novelty, or economy theorem | [S2 Newton/Bessel result](observer-driven-scale-chart-compiler/03-analytic-germ-newton-bessel.md); next require a second representation family and executable same-information Wolfram comparison before any claim expansion |
