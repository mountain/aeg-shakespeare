...
| [`finite-le-semantic-evaluator/`](finite-le-semantic-evaluator/) | T1 / EXPAND in frozen fragment | exact real single-exponential rational-rate limits with derived charts, domain witnesses, residuals, and deterministic C2 replay; no raw advantage over the same-information SymPy baseline | [results](finite-le-semantic-evaluator/01-results.md) and [workstream](../workstreams/finite_le_semantics/); next freeze paired-source chart discovery/transport, not a general transseries or surreal runtime |
