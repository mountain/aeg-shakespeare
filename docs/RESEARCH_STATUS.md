...
| [`boltzmann-bbgky-h-theorem`](../sonnet/boltzmann-bbgky-h-theorem/README.md) | through independent Phases 1J-A/C2 and 1J-B2: ordered finite collision action and nonlinear response cocycle; whole-event collision-flux contract; fixed-molecule root-visible mark with ten exact pushforward, Fubini, topology, and mark-cost certificates | prove a marked operation-sequence summation with an explicit test class and retained gain, then identify physical flux; C3 covector selection and the logarithmic tail stay separate, and no trace, continuum response estimate, or hard-sphere H theorem is claimed |
