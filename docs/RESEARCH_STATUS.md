...
| [`boltzmann-bbgky-h-theorem`](../sonnet/boltzmann-bbgky-h-theorem/README.md) | through independent Phases 1J-A/C2 and 1J-B3: finite collision response cocycle; whole-event flux contract; fixed-molecule mark; bounded formal marked family absolutely summable in total variation with thirteen exact routing, series, tail, error, and red-team certificates | identify the formal signed molecule family with a declared microscopic collision current, including orientation and separately typed residual currents; C3 covector selection and the logarithmic tail stay separate, and no trace, physical continuum response estimate, or hard-sphere H theorem is claimed |
