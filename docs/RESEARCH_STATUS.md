...
| [`boltzmann-bbgky-h-theorem`](../sonnet/boltzmann-bbgky-h-theorem/README.md) | through independent Phases 1J-A/C2 and 1J-B4: finite collision response cocycle; whole-event flux contract; fixed/global formal marking; signed-current audit with twelve exact path/current and no-go certificates; source equations (5.63) and (7.15) supply exact endpoint/local-atom ingredients but not physical identification | prove the one-layer marked Penrose insertion identity on a path-resolved lift, then construct separately typed truncation, geometry, `Err_2`, and terminal residual currents; C3 covector selection and the logarithmic tail stay separate, and no physical continuum response estimate or hard-sphere H theorem is claimed |
