...
| [`boltzmann-bbgky-h-theorem`](../sonnet/boltzmann-bbgky-h-theorem/README.md) | through Phase 1J-A/C2: exact finite adapters, weak/mild cells, time-reversal obstruction, hidden Lyapunov cone, charted response ledger, ordered collision action, set-valued forgotten response, and nonlinear response cocycle | Phase 1J-B Deng molecule/cutting response estimate in weak collision-flux topology; C3 covector selection stays held out, and no hard-sphere H theorem is claimed |
