...
| [`boltzmann-bbgky-h-theorem`](../sonnet/boltzmann-bbgky-h-theorem/README.md) | through independent Phases 1J-A/C2 and 1J-B: ordered finite collision action, set-valued forgotten response, nonlinear response cocycle, whole-event collision-flux contract, nine exact clipped/tail H-budget certificates, and a primary-source proof boundary | prove a bounded marked-molecule flux lift before the logarithmic tail; C3 covector selection stays separate, and no trace, continuum response estimate, or hard-sphere H theorem is claimed |
