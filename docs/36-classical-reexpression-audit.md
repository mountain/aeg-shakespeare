...
calibration; complete coverage of `tests/classical/` on main at commit
`e25e575`. It changes API ownership and vocabulary without promoting a new
Mathematical Core or Theory Map object.
...
The present audit therefore re-reads **every one of the 30 executable essays**
...
**Family judgment.** The exact relation-first programme survives. The new core
narrows the word `quotient` in the even-power essay and strengthens the
two-frequency negative control: algebraic/function refinement is downstream of
task adequacy and does not choose itself.
...
ledger fail-closed when the directory gains a new executable essay. It checks
coverage only; it does not certify any mathematical claim.

## 10. Recalibration order

The work should proceed in narrow, independently reviewable stages.

### R0 — complete inventory and one unambiguous correction

- record the present 30-file/65-test audit;
...
### R1 — A/M and oscillator calculation contracts
