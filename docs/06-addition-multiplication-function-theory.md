...
### 7.1 Literal polynomial boundary

Here `polynomial` has its ordinary finite algebraic meaning:

\[
P_{\le n}(a)
=
\left\{\sum_{k=0}^{n}c_k a^k\right\}.
\]

It means finite support, finite degree, and non-negative integer powers of the
declared indeterminate `a`.  The constructor requires `a` to be a symbolic
indeterminate and stores exactly the basis `1,a,...,a**n`.

This finite module is a declared A/M-invariant calibration family.  It is not
the family *generated by every allowed A/M construction*.  In particular, the
power-weight expressions

\[
a^\nu e^{(w-\nu)v},
\]

the resonant logarithmic extensions, path-flow integrals, formal or convergent
power series, and any future analytic closure are separate function-language
objects.  Some special parameter values may make one expression polynomial;
that coincidence does not identify the surrounding families.
