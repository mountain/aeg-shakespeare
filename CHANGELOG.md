...
- certifies the branch-locus analysis: the local decoder chart fails `0/0` at `U = +/-1` as a chart artifact while the exact energy identity keeps two distinct Cartesian states there, so the physical cover is the nontrivial unramified double cover of the real carrier loop; the Z2 mark flips once per traverse (sheet transport through `q_x = 0`) and the marked state closes after two traverses (`2 omega_A`, the physical pendulum period);
